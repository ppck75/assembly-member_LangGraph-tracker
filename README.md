...
- **Fan-out/Fan-in 병렬 분석 파이프라인**:
  1. 입력값 정리 (사용자 요청 및 옵션 정규화)
  2. 의원 기본정보 조회 (재임 대수, 정당, 선거구 확인)
  3. 의원 기본정보 확인 후 3개 작업을 병렬 실행
     - 발의법안 조회
     - 표결 상세 조회
     - 최근 이슈 검색
  4. 발의법안 조회 완료 후 2개 분석을 병렬 실행
     - 입법 관심 분야 분석
     - 공동발의 네트워크 분석
  5. 표결 상세 조회 완료 후 정당 다수 입장 일치도 분석 및 표결 해석 메모 생성
  6. 입법 관심 분야, 공동발의 네트워크, 표결 해석, 최근 이슈가 모두 완료되면 핵심 브리핑 생성
  7. 최종 결과 정리 (화면 표시 형태로 정렬)
- **병렬 상태 병합**: 여러 노드가 동시에 오류 메시지나 LLM quota 상태를 갱신해도 충돌하지 않도록 LangGraph state reducer를 적용
- **노드 내부 최적화**: `get_all_member_votes`, `analyze_party_alignment` 내부의 `BILL_ID`별 API 반복 호출은 `async def`/`await` 기반 비동기 I/O로 처리
...
```
                ...
                │   │  • 24시간 캐시 시스템 (의원 목록, 발의법안, 표결 상세, 뉴스)             │   │
                │   │  • async/await 기반 BILL_ID별 표결 상세 비동기 조회                      │   │
                │   └────────────────────────────────────────────────────────────────────────┘   │
                │                                                                                │
                └───────────────────────────────────────┬────────────────────────────────────────┘
                                                        │
                ...
                │             ▼                         ▼                         ▼              │
                │   ┌─ 병렬 A ──────────────┐ ┌─ 병렬 B ──────────────┐ ┌─ 병렬 C ──────────────┐ │
                │   │ 발의법안 조회         │ │ 표결 상세 조회        │ │ 최근 이슈 검색        │ │
                │   │ search_member_bills   │ │ get_all_member_votes  │ │ search_recent_news    │ │
                │   └──────────┬────────────┘ └──────────┬────────────┘ └──────────┬────────────┘ │
                │              │                         │                         │              │
                │      ┌───────┴────────┐                ▼                         │              │
                │      ▼                ▼       ┌──────────────────────┐           │              │
                │ ┌───────────────┐ ┌───────────────┐ │ 정당 일치도 분석 │           │              │
                │ │ 입법 관심 분석│ │ 공동발의 분석│ │ party_alignment  │           │              │
                │ │ legislative   │ │ cosponsor     │ └──────────┬───────┘           │              │
                │ │ interests     │ │ network       │            ▼                   │              │
                │ └───────┬───────┘ └───────┬───────┘ ┌──────────────────────┐      │              │
                │         │                 │         │ 표결 해석 메모 생성  │      │              │
                │         │                 │         │ interpret_vote_stats │      │              │
                │         │                 │         └──────────┬───────────┘      │              │
                │         └─────────────────┴────────────────────┼──────────────────┘              │
                │                                                ▼                                 │
                │                        ┌─ Fan-in ───────────────────────┐                         │
...
| 실행 구간 | 노드명 | 실행 방식 | 역할 | 주요 의존성 |
|------|--------|------|------|------|
| 1 | `normalize_user_request` | 선행 실행 | 사용자 요청 및 분석 옵션 정규화 | 사용자 입력 |
| 2 | `get_member_info` | 선행 실행 | 의원 기본정보, 재임 대수, 분석 가능 대수 산출 | 정규화된 의원명 |
| 3-A | `search_member_bills` | 병렬 실행 | 대표발의·공동발의 법안 수집 및 처리결과/소관위원회 통계 계산 | 의원 기본정보 |
| 3-B | `get_all_member_votes` | 병렬 실행 | 본회의 표결 상세 조회 및 찬성·반대·기권·불참 분류 | 의원 기본정보 |
| 3-C | `search_recent_member_news` | 병렬 실행 | Google News RSS 기반 최근 이슈 수집 및 요약 | 의원 기본정보 |
| 4-A-1 | `analyze_legislative_interests` | 병렬 실행 | 발의법안 기반 입법 관심 분야 분석 | 발의법안 조회 결과 |
| 4-A-2 | `analyze_cosponsor_network` | 병렬 실행 | 공동발의 파트너 및 협업 패턴 분석 | 발의법안 조회 결과 |
| 4-B-1 | `analyze_party_alignment` | 순차 실행 | 같은 정당 의원 다수 입장과의 일치·이탈 분석 | 표결 상세 조회 결과 |
| 4-B-2 | `interpret_vote_statistics` | 순차 실행 | 조회 범위와 불참 비중을 고려한 표결 해석 메모 생성 | 정당 일치도 분석 결과 |
| 5 | `generate_activity_briefing` | Fan-in 이후 실행 | 입법·공동발의·표결·최근 이슈를 종합한 핵심 브리핑 생성 | 4개 분석 분기 완료 |
| 6 | `summarize_member_activity` | 최종 실행 | 화면 표시용 요약 결과 정리 | 핵심 브리핑 및 전체 분석 결과 |

병렬화 기준은 **서로의 산출물을 기다릴 필요가 없는 작업만 동시에 실행**하는 것입니다. 예를 들어 표결 상세 조회는 발의법안 조회 결과를 필요로 하지 않으므로 `search_member_bills`와 동시에 실행됩니다. 반면 정당 일치도 분석은 표결 상세 조회 결과가 있어야 하므로 `get_all_member_votes` 이후에만 실행됩니다.
